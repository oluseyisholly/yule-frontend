...
type WishlistDetailContentTab = "gifts" | "hangout";
...
function mapParticipant(
  ...
  const fullName =
    `${actor.firstName} ${actor.lastName}`.trim() || "Participant";
  ...
  return (
    <div className="flex min-w-0 items-center gap-2">
      <UserAvatar
        ...
        bgColor={participant.bg}
        textColor={participant.color}
        className="size-8 shrink-0 text-[10px] font-semibold"
        title={participant.name}
      />
...
export default function WishListDetailsScreen({
  ...
  const [activeContentTab, setActiveContentTab] =
    useState<WishlistDetailContentTab>("gifts");

  const deleteWishlistEventMutation = useDeleteWishlistEventMutation();
  const { data, isLoading, isError, refetch } =
    useWishlistEventQuery(wishlistEventId);
  const {
    data: wishlistHangoutResponse,
    isError: isHangoutError,
    refetch: refetchWishlistHangout,
  ...
  const detail = useMemo(() => {
    ...
    const participants = (record.event.participants ?? [])
      ...
      .filter((participant): participant is DetailParticipant =>
        Boolean(participant),
      );
  ...
  const sourceFlowSelection = editFlowSelection?.lastVisitedStep
    ? editFlowSelection
    : createFlowSelection;
  const resumeStep =
  ...
  const canClaimWishlist =
    isParticipantDetail && !canManageDetail && isOngoingWishlist;
  ...
  const isActiveGiftsError = canManageDetail
    ? isGiftsError
    : isClaimedGiftsError;
  const activeWishlistHangout =
    wishlistHangoutResponse?.data ?? data?.data?.hangoutEvent ?? null;
  const canViewWishlistHangout = useMemo(() => {
    if (!data?.data?.hangoutEventId?.trim() || !activeWishlistHangout) {
      return false;
    }
    ...
    const payerEventContactId =
      activeWishlistHangout.payerParticipant?.eventContact?.id?.trim() ||
      activeWishlistHangout.payerParticipant?.eventContactId?.trim() ||
      null;
    const payerUserId =
      activeWishlistHangout.payerParticipant?.user?.id?.trim() || null;
    const normalizedCurrentContactId = currentContactId?.trim() || null;
    const normalizedCurrentUserId = authUser?.id?.trim() || null;

    return Boolean(
      (normalizedCurrentContactId &&
        payerEventContactId &&
        normalizedCurrentContactId === payerEventContactId) ||
        (normalizedCurrentUserId &&
          payerUserId &&
          normalizedCurrentUserId === payerUserId),
    );
  }, [
    activeWishlistHangout,
    authUser?.id,
    canManageDetail,
    currentContactId,
    data?.data?.hangoutEventId,
  ]);
  const hasHangoutSection = canViewWishlistHangout;
  const hasGiftsSection = useMemo(() => {
    const fallbackCount = data?.data?.items?.length ?? 0;
    const queriedCount = activeGiftPageResponse?.total ?? 0;

    return queriedCount > 0 || fallbackCount > 0;
  }, [activeGiftPageResponse?.total, data?.data?.items?.length]);
  const shouldShowContentTabs = hasHangoutSection && hasGiftsSection;
  ...
  useEffect(() => {
    if (shouldShowContentTabs) {
      return;
    }

    if (hasGiftsSection) {
      setActiveContentTab("gifts");
      return;
    }

    if (hasHangoutSection) {
      setActiveContentTab("hangout");
    }
  }, [hasGiftsSection, hasHangoutSection, shouldShowContentTabs]);
  ...
  const handleOpenEditFlow = () => {
    ...
    router.push(
      buildWishListFlowHref(nextStep, "edit", detail.eventId, wishlistEventId),
    );
  };
  ...
  const handleDelete = async () => {
    try {
      const response =
        await deleteWishlistEventMutation.mutateAsync(wishlistEventId);
  ...
  const giftTableData: TableData<GiftRow> = {
        ...
        ? [
            {
              ...
              cellClassName: "px-3 py-3",
              render: (row: GiftRow) => (
                <AssignedParticipantCell
                  participant={row.assignedParticipant}
                />
              ),
            },
          ]
    ...
    emptyState:
      isActiveGiftsLoading || isActiveGiftsFetching ? (
        ...
        <div className="flex flex-col gap-5 pb-5 sm:pb-6">
          ...
          <div className="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(280px,0.9fr)]">
            <div className="rounded-[20px] border border-[#EEEAF7] bg-white p-4 sm:p-5">
                    ...
                    <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                      <div className="relative w-full sm:w-[220px]">
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            setGiftSearchValue(event.target.value)
                          }
              ...
              {shouldShowHangoutSection && activeWishlistHangout ? (
                <div className="space-y-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <h2 className="text-[16px] font-[600] text-[#000000]">
                      Hangout
                  ...
                  <div className="overflow-x-auto">
                    <div className="min-w-[760px] rounded-[16px] border border-[#F0EEFF] bg-white px-4 py-4">
                      <div className="grid grid-cols-[92px_minmax(220px,1.4fr)_minmax(140px,0.9fr)_minmax(120px,0.8fr)] items-center gap-4">
                        <div className="flex h-[84px] w-[84px] items-center justify-center overflow-hidden rounded-[12px] bg-[#F3EFFB]">
                          {activeWishlistHangout.imageUrl ? (
                            <img
                              src={activeWishlistHangout.imageUrl}
                              alt={
                                activeWishlistHangout.eventCenterName ||
                                activeWishlistHangout.event.title ||
                                "Selected hangout"
                              }
                              className="h-full w-full object-cover"
                            />
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 truncate text-[12px] text-[#7D7D7D]">
                            {activeWishlistHangout.location ||
                        ...
                        <div className="min-w-0">
                          ...
                          {activeWishlistHangout.payerParticipant
                            ?.eventContact ? (
                            <div className="mt-1 flex items-center gap-2 min-w-0">
                              <UserAvatar
                                name={
                                  `${activeWishlistHangout.payerParticipant.eventContact.firstName} ${activeWishlistHangout.payerParticipant.eventContact.lastName}`.trim() ||
                                  activeWishlistHangout.payerParticipant
                                    .eventContact.email
                                }
                                initials={toInitials(
                                ...
                                imageUrl={
                                  activeWishlistHangout.payerParticipant
                                    .eventContact.profileUrl ?? null
                                }
                                bgColor={
                                  getParticipantStyle(
                                    activeWishlistHangout.payerParticipant
                                      .eventContact.id ||
                                      activeWishlistHangout.payerParticipant.id,
                                  ).bg
                                }
                                textColor={
                                  getParticipantStyle(
                                ...
                                className="size-7 text-[10px] font-semibold"
                                title={
                                  `${activeWishlistHangout.payerParticipant.eventContact.firstName} ${activeWishlistHangout.payerParticipant.eventContact.lastName}`.trim() ||
                                  activeWishlistHangout.payerParticipant
                                    .eventContact.email
                            ...
                            <p className="mt-1 truncate text-[13px] font-medium text-[#1E1E1E]">
                              —
                            </p>
                          )}
                        </div>
                        ...
                        {/* <div className="min-w-0">
                          ...
                          <p className="mt-1 text-[13px] font-medium text-[#1E1E1E]">
                            {formatDate(data.data.hangoutEvent.checkInDate)}
                          </p>
                        </div>
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-[13px] font-medium text-[#1E1E1E]">
                            {formatDate(data.data.hangoutEvent.checkOutDate)}
                          </p>
                        </div> */}
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-[13px] font-semibold text-[#1E1E1E]">
                            {formatCurrency(activeWishlistHangout.amount)}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-end">
                    <Button
                      ...
                      onClick={() =>
                        router.push(
                          `/dashboard/hangouts/${encodeURIComponent(activeWishlistHangout.eventId)}?backHref=${encodeURIComponent(`/dashboard/wish-list/${wishlistEventId}`)}`,
                        )
                      }
                      className="h-[40px] rounded-full px-5 text-sm font-medium"
                    >
                      View Hangout
                    </Button>
                  </div>
